...
$pdo = connect();
$req=$pdo->query('SELECT id_chalet From chalet WHERE massif="Valais"');
...
if($last_pages != 1) {
    if($page_num > 1) {
        ...
        $pagination .= '<a href = "chaletValais.php?page=' . $page_precedente. '" class="lien_pagination">Précédent</a>&nbsp; &nbsp;';
         for($i = $page_num - $nombre_pages_max_gauche_et_droite; $i < $page_num; $i++) {
             if($i > 0) {
                 $pagination .= '<a href ="chaletValais.php?page=' . $i . '" class="lien_pagination"> ' . $i .'</a>&nbsp;';
             }
         }
    }
    ...
    if($page_num != $last_pages) {
        ...
        $pagination .= '<a href =" chaletValais.php?page=' . $page_suivante .'" class="lien_pagination">&nbsp; &nbsp; Suivant</a>';
    }
}

//Sélection et affichage du template PHTML.
$template = 'chaletValais';
